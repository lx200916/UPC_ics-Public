...
def icsCreateAndSave():
    ...
    for classInfo in classInfoList:
        ...
        className = classInfo["className"] + "|" + list(classTimeList.values())[i]["name"] + "|" + classInfo[
            "classroom"]
    ...
    if (str(info) != '[]'):
        ...
        if '-' in weeks:
            if ',' in weeks:
                week = weeks.split(',')
                for aweek in week:
                    clasinfo['week']['startWeek'] = int(aweek[0:aweek.find('-')])
                    clasinfo['week']['endWeek'] = int(aweek[aweek.find('-') + 1:])
                    classinfos.append(clasinfo)
            else:
